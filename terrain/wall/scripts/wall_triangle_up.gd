	...
	for x in range(grid_map.GRID_SIZE):
		for z in range(grid_map.GRID_SIZE):
			...
			if current_tile._top_type == GridTileData.TileTopType.DIAGONAL_SLOPE:
				...
				tile_mesh_data.append({ "x": x, "y": current_height, "z": z, "rotation_idx": current_tile._rotation})
				tile_mesh_data.append({ "x": x, "y": current_height+1, "z": z, "rotation_idx": _rotation})
			...
			if current_tile._top_type == GridTileData.TileTopType.V:
				...
				var _rotation_2 = wrapi(current_tile._rotation + 3, 0, 4)
